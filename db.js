...
const update_prev_content_vdraft2 = async (data) => {


  const {
      content, plid, model, prev, length_llm
  } = data;
  ...
  try {
      ...
      if (result.rows.length > 0) {
          if (!(length_llm < 3) || length_llm >= 3) {
              ...
              try {
                  const result = await pool.query(query_nclogs, ["DRAFT_FINAL", plid]);
...
const getVerificDr = async (data) => {
  const { process_name } = data;
  const query = "Select nc.*, pl.* from nc_processed_content nc, nc_process_logs pl  where pl.id in (select id from nc_process_logs where processor =$1) and pl.status = 'DRAFT_VERIFY' and nc.plid in (select id from nc_process_logs where processor = $1) ORDER BY  pl.nc_date_created ASC limit 1";
...
const getDrafts = async (data) => {
  const { process_name } = data;
  const query = "Select nc.*, pl.*  from  nc_processed_content nc , nc_process_logs pl  where pl.id in (select id from nc_process_logs where processor = $1) and pl.status = 'DRAFT_START' and nc.plid in (select id from nc_process_logs where processor = $1) ORDER BY pl.nc_date_created ASC limit 1";
  try {

        const result = await pool.query(query, [process_name]);

      console.log("@GETDRAFTS @GETDRAFTS @GETDRAFTS =");
      if (result.rows.length > 0) {
          console.log(result.rows[0].results);          
          return result.rows;
      } else {
          // return null;
          console.log("NO ROWS 4 PROCESS DRAFT_START ");
          throw new Error(`Nothing to process for this processor`);
      }
  } catch (error) {
      console.log("error get drafts");
      console.log(error);
      throw new Error(`Nothing to process for this step`);
  }
};
...
const pushV2Draftsinfo = async (data) => {
  const {
      content,   plid,   model
  } = data;
  console.log("++++++pushvdraftsinfo++++++");
  ...
  try {
      ...
      console.log("==================prev_content.rows[0] after selecting previous content ========================");
      console.log(prev_content.rows[0].results);
      prev = prev_content.rows[0].results_3;
  } catch (error) {
      console.log("error get drafts");
      console.log(error);
      // return something and abort
  ...
  if (length_llm < 3) {
      ...
      console.log("newupdate 2 length_llm < 3  newupdate 2 length_llm < 3 =");
      console.log(length_llm);
      let newupdate = await update_prev_content_vdraft2({
          prev,
          content,
          plid,
          model,
      ...
      if(prev.indexOf("Llama-4-Scout") > -1 && prev.indexOf("Instruct-Turbo") > -1 && prev.indexOf("mistralai/Mistral") > -1){
              ...
              const query = `UPDATE nc_process_logs
                SET status = $1 WHERE id = $2 and status = 'DRAFT_VERIFY_2'
                RETURNING *
              `;

              try {
                  const result = await pool.query(query, ["DRAFT_FINAL", plid]);
                  console.log("@@ after update of status to draft_verify!");
                  console.log(result.rows[0]);
              } catch (error) {
                  console.log("Error in update of nc logs to DRAFT_FINAL");
                  throw new Error(`Error during DRAFT_FINAL update`);
...
const getVerificDr2 = async (data) => {
  ...
  const { process_name } = data;
  const query = "Select nc.*, pl.* from nc_processed_content nc, nc_process_logs pl  where pl.id in (select id from nc_process_logs where processor =$1) and pl.status = 'DRAFT_VERIFY_2' and nc.plid in (select id from nc_process_logs where processor =$1) ORDER BY  pl.nc_date_created ASC limit 1";
